...
namespace Match3
{
    public class GameGrid : MonoBehaviour
    {
        ...
        public void FreezeRandomPieces(int count)
        {
            int frozenCount = 0;

            // Будем итерировать до тех пор, пока не создадим нужное количество замороженных элементов 
            while (frozenCount < count)
            {
                int x = Random.Range(0, xDim);
                int y = Random.Range(0, yDim);

                // Пропустите цикл, если элемент уже заморожен
                if (_pieces[x, y].Type == PieceType.Frozen)
                {
                    continue;
                }

                // Замените тип элемента соответствующим образом перед замораживанием его
                if (_pieces[x, y].Type == PieceType.Normal)
                {
                    // Уничтожьте текущий нормальный объект перед созданием нового замороженного
                    GameObject pieceToFreeze = _pieces[x, y].gameObject;
                    Destroy(pieceToFreeze);

                    SpawnNewPiece(x, y, PieceType.Frozen);
                    frozenCount++;
                }
            }
        }
        public bool IsFilling { get; private set; }
        public void UnfreezeAdjacent(GamePiece piece)
        {
            for (int xOffset = -1; xOffset <= 1; xOffset++)
            {
                for (int yOffset = -1; yOffset <= 1; yOffset++)
                {
                    // Если смещение не является вертикальным или горизонтальным, пропустите этот шаг
                    if (Mathf.Abs(xOffset) == Mathf.Abs(yOffset))
                    {
                        continue;
                    }

                    int newX = piece.X + xOffset;
                    int newY = piece.Y + yOffset;

                    if (newX >= 0 && newX < xDim && newY >= 0 && newY < yDim)
                    {
                        if (_pieces[newX, newY].Type == PieceType.Frozen)
                        {
                            // Уничтожьте текущий замороженный объект перед созданием нового
                            GameObject frozenPiece = _pieces[newX, newY].gameObject;
                            Destroy(frozenPiece);

                            SpawnNewPiece(newX, newY, PieceType.Empty);
                        }
                    }
                }
            }
